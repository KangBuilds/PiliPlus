...
abstract final class SearchHttp {
  ...
  static Future<LoadingState<SearchSuggestModel>> searchSuggest({
    ...
    return parseSuggestResponse(res.data);
  }

  @visibleForTesting
  static LoadingState<SearchSuggestModel> parseSuggestResponse(dynamic data) {
    if (data is Map && data['code'] == 0) {
      try {
        return Success(SearchSuggestModel.fromJson(data['data']['result']));
      } catch (_) {
